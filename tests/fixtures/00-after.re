...
let getMonthsRange = (start, monthsDuration) => {
  let months = ref([]);
  let lastMonth =
    start |> Moment.add(~duration=duration(monthsDuration, `months));

  let month =
    ref(
      start
      |> Moment.subtract(~duration=duration(1, `months))
...
let isDifferentYear = month =>
  Moment.year(moment(month)) != Moment.year(momentNow());

let isCurrentMonth = month => Moment.format("YYYY-MM", momentNow()) == month;

let isInPast = month =>
  (moment(month) |> Moment.startOf(`month))
  ->(Moment.isBefore(currentMonth()));
